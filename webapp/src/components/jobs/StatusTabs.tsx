...
const TAB_STYLES: Record<FilterStatus, string> = {
  ...
  skipped: 'text-slate-500 border-slate-200 hover:border-slate-400',
  closed: 'text-slate-500 border-slate-200 hover:border-slate-400',
  archived: 'text-slate-400 border-slate-200 hover:border-slate-300',
}
...
const ACTIVE_TAB_STYLES: Record<FilterStatus, string> = {
  ...
  skipped: 'bg-slate-500 text-white border-slate-500',
  closed: 'bg-slate-500 text-white border-slate-500',
  archived: 'bg-slate-400 text-white border-slate-400',
}
...
export function StatusTabs({ jobs, active, onChange }: Props) {
  ...
  const totalActive = jobs.filter(
    (j) => j.status !== 'archived' && j.status !== 'closed' && j.status !== 'skipped'
  ).length

  const tabs: FilterStatus[] = ['needs_apply', 'applied', 'replied', 'interview', 'offer', 'rejected', 'skipped', 'closed', 'archived', 'all']
